...
def crop_lip(frame, mode):
    ...
    for i, rect in enumerate(rects):
        ...
        center = [
            int(0.5 * (shape[62][1] + shape[66][1])),
            int(0.5 * (shape[62][0] + shape[66][0])),
        ]
        if mode == "second":
            angle.append(
                np.arctan((shape[54][1] - shape[48][1]) / (shape[54][0] - shape[48][0]))
                * 180
                / np.pi
            )
...
while cap.isOpened():
    ...
    if mode == "first":
        ...
        cv2.putText(
            frame,
            "To quit this program, press ESC, To start saving, press 1",
            (15, 20),
            demo_font,
            0.7,
            (0, 255, 0),
    ...
    if mode == "third":
        ...
        cv2.putText(
            frame,
            "Predicted Label: {}({:2.1%})".format(
                word_class_en[test_labels[0]][:-1], test_prob[0][test_labels[0]]
            ),
            (15, 20),
            demo_font,
            0.7,
    ...
    if mode == "second":
        ...
        lip_frame = frame[
            center[0] - h_len : center[0] + h_len, center[1] - h_len : center[1] + h_len
        ]
        ...
        cv2.rectangle(
            frame, (0, 0), (frame.shape[1] - 2, frame.shape[0] - 2), (0, 0, 255), 2
        )
        cv2.putText(
            frame, "frame : {}".format(cnt), (15, 20), demo_font, 0.7, (0, 255, 0), 2
        )
        ...
        if cnt == max(25, end_i[j] - start_i[j]):
            ...
            if count >= N_frame:
                if count % 2 == 0:
                    ...
                    frame_N = range(
                        int(count / 2 - (((N_frame - 1) / 2) * frperiod)),
                        int(count / 2 + (((N_frame - 1) / 2) * frperiod) + 1),
                        frperiod,
                    )
                else:
                    frperiod = int((count + 1) / N_frame)
                    frame_N = range(
                        int((count + 1) / 2 - (((N_frame - 1) / 2) * frperiod)),
                        int((count + 1) / 2 + (((N_frame - 1) / 2) * frperiod) + 1),
                        frperiod,
                    )
            ...
            with tf.Graph().as_default() as g:
                ...
                try:
                    ...
                    image, test_labels, test_prob = sess.run(
                        [image, top_k_predict_op, probabilities_op]
                    )
            ...
            print(
                "Predicted Label: {}({:2.1%})".format(
                    word_class[test_labels[0]], test_prob[0][test_labels[0]]
                )
            )
            for i in range(len(word_class)):
                if test_prob[0][i] > 0.05:
                    print(
                        "Label {}: {:2.1%}".format(
                            word_class_en[i].ljust(9), test_prob[0][i]
                        )
                    )
