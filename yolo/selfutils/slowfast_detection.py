...
)  # Another option is slowfast_r50_detection, slow_r50_detection
...
def get_person_bboxes(inp_img, predictor):
    ...
    predicted_boxes = boxes[
        np.logical_and(classes != -1, scores > 0.5)
...
def main(args):
    ...
    label_map, allowed_class_ids = AvaLabeledVideoFramePaths.read_label_map(
        "selfutils/ava_action_list.pbtxt"
    )
    ...
    time_stamp_range = range(
        0, int(encoded_vid.duration // 1), 1
    ...
    for time_stamp in time_stamp_range:
        ...
        inp_imgs = encoded_vid.get_clip(
            time_stamp, time_stamp + clip_duration  # start second  # end second
        )
        ...
        inputs, inp_boxes, _ = ava_inference_transform(
            inp_imgs, predicted_boxes.numpy(), crop_size=args.imsize
        )
        ...
        out_img_pred = video_visualizer.draw_clip_range(
            inp_imgs, preds, predicted_boxes, repeat_frame=1
        )
        gif_imgs += out_img_pred

    print("Finished generating predictions.")
    print(
        "total cost: {:.3f}s, video clips length: {}s".format(
            time.time() - a, len(time_stamp_range)
        )
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", type=str, default="/data/VAD/SHTech/testing/videos/01_0015.mp4"
    )
